...
def generate_lidar_blueprint(blueprint_library): 
    lidar_blueprint = blueprint_library.find('sesor.lidar.ray_cast_semantic')
...
def semantic_lidar_data(point_cloud_data): 
    distance_name_data = {}
    for n in point_cloud_data: 
        position = value_list.index(detection.object_tag)
        distance = "Apply math.sqrt function here"((detection.point.x ** 2) + (detection.point.y ** 2) + (detection.point.z ** 2))
